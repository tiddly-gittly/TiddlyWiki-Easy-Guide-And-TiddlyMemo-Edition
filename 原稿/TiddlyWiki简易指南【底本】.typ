...
#image("./media/image1.png", width: 0.525in, height: 0.5958333333333333in)#strong[TiddlyWiki（太微）简易指南]

此文件所在的Github仓库：#link("https://github.com/tiddly-gittly/TiddlyWiki-Easy-Guide-And-TiddlyMemo-Edition")[tiddly-gittly/TiddlyWiki-Easy-Guide-And-TiddlyMemo-Edition]
...
= #strong[目录]
<目录>
#link(<前言>)[前言 #link(<前言>)[ii]]

#link(<一创建自己的知识库>)[一、创建自己的知识库 #link(<一创建自己的知识库>)[1]]

#link(<一tiddlywiki-简介>)[（一）TiddlyWiki 简介 #link(<一tiddlywiki-简介>)[1]]

#link(<二下载与安装>)[（二）下载与安装 #link(<二下载与安装>)[3]]

#link(<三太微的基本介绍>)[（三）太微的基本介绍 #link(<三太微的基本介绍>)[4]]

#link(<二开始使用知识库>)[二、开始使用知识库 #link(<二开始使用知识库>)[15]]

#link(<一创建第一个条目笔记>)[（一）创建第一个条目笔记 #link(<一创建第一个条目笔记>)[15]]

#link(<二文件目录侧边栏>)[（二）“文件目录”侧边栏 #link(<二文件目录侧边栏>)[36]]

#link(<三筛选器与双链功能>)[（三）筛选器与双链功能 #link(<三筛选器与双链功能>)[38]]

#link(<三如何使用tiddlywiki构建笔记系统>)[三、如何使用TiddlyWiki构建笔记系统？ #link(<三如何使用tiddlywiki构建笔记系统>)[53]]

#link(<一时间序列记录法>)[（一）时间序列记录法 #link(<一时间序列记录法>)[54]]

#link(<二主题渐构笔记法>)[（二）主题渐构笔记法 #link(<二主题渐构笔记法>)[56]]

#link(<三关系图谱或图数据结构>)[（三）关系图谱或图数据结构 #link(<三关系图谱或图数据结构>)[60]]

#link(<四线性写作的形式>)[（四）线性写作的形式 #link(<四线性写作的形式>)[64]]

#link(<五渐进写作增量写作>)[（五）渐进写作（增量写作） #link(<五渐进写作增量写作>)[65]]

#link(<六自动化无压记录>)[（六）自动化无压记录 #link(<六自动化无压记录>)[66]]

#link(<七随手记录动态生成>)[（七）随手记录，动态生成 #link(<七随手记录动态生成>)[68]]

#link(<八卡片笔记写作法>)[（八）卡片笔记写作法 #link(<八卡片笔记写作法>)[69]]

#link(<九para个人知识管理>)[（九）PARA个人知识管理 #link(<九para个人知识管理>)[69]]

#link(<十书籍与书籍的分卷>)[（十）书籍与书籍的分卷 #link(<十书籍与书籍的分卷>)[70]]

#link(<十一分享和发布功能>)[（十一）分享和发布功能 #link(<十一分享和发布功能>)[71]]

#link(<十二数据安全性与备份>)[（十二）数据安全性与备份 #link(<十二数据安全性与备份>)[71]]

#link(<四为知识库拓展新的功能与特性>)[四、为知识库拓展新的功能与特性 #link(<四为知识库拓展新的功能与特性>)[74]]

#link(<一markdown插件>)[（一）Markdown插件 #link(<一markdown插件>)[75]]

#link(<二反向链接插件>)[（二）反向链接插件 #link(<二反向链接插件>)[75]]

#link(<三toc自动化目录插件>)[（三）TOC自动化目录插件 #link(<三toc自动化目录插件>)[76]]

#link(<四回收站插件>)[（四）回收站插件 #link(<四回收站插件>)[78]]

#link(<五随机漫游条目与快照插件>)[（五）随机漫游条目与快照插件 #link(<五随机漫游条目与快照插件>)[79]]

#link(<六与supermemo互动>)[（六）与SuperMemo互动 #link(<六与supermemo互动>)[80]]

#link(<五帮助与更多>)[五、帮助与更多 #link(<五帮助与更多>)[82]]

#link(<一桌面端timimi的安装使用>)[（一）桌面端Timimi的安装使用 #link(<一桌面端timimi的安装使用>)[84]]

#link(<二配置太微的中文语言>)[（二）配置太微的中文语言 #link(<二配置太微的中文语言>)[89]]

== 一、创建自己的知识库
<一创建自己的知识库>
很久很久以前，在一条河流里，生活着许多许多的小鱼儿（Tiddlers），它们快活地在故事河（Story
River）里游来游去。这些小鱼儿每一个都有自己的名字（条目名），自己的种类（条目内容类型），它携带的信息组成了鱼儿美丽的身体与骨架，附加的字段构成了它美丽的鳞片。

=== （一）TiddlyWiki 简介
<一tiddlywiki-简介>
TiddlyWiki
是一个宝藏软件，但可惜的是有些人没有体会到这一点就放弃了。我们把互联网看作一个巨大的知识库，它由每个网页组成。使用TiddlyWiki进行信息组织，就是编辑一个一个的网页（条目），这些网页（条目）组成自己的知识库。

「太微」是我们对 TiddlyWiki
的中文称呼。它是一款独特的非线性笔记本，用于捕获、组织和分享复杂信息。可用它来建立个人知识库，或者任务管理。记录闪过你大脑的每一个想法，或者建立一个灵动的自适应式的网站。
...
我们会使用TidGi（太记）作为本文的全部主线来为您介绍如何使用TiddlyWiki，您以后会见到各种各样的TiddlyWiki，比如：TiddlyMemo（墨屉）、TiddlyDesktop（TiddlyWiki官方的桌面启动器）、TiddlyWiki
Xp（一个快速体验太微的｢太微模版｣）等等。这些都是不同愿景和功能的TiddlyWiki。其中TidGi（太记）是一个应用程序而非系统或框架，使用简单且功能丰富。
...
#strong[普通条目]，包含：tile 条目标题、tag 标签、text 条目内容、type
内容类型、field 栏位

+ tag
  标签包括系统标签以及普通标签，其中系统标签被【太微】用来给条目提供特殊的行为。

+ field 栏位包含field name 字段名、field value
  字段值。条目字段：可以想象成一个个的空位，能够放置除了文本信息之外的内容，即自定义的拓展的小鱼内容。

#strong[系统条目]：如果Tiddlers的标题以特殊字符串开头，则将其归类为系统Tiddlers。系统条目大致有一下几种类型。插件、默认、模板、样式、配置等等。

- 插件（条目组）：在内部，插件是一些条目被打包成可被安装、拷贝、禁用或删除的单一条目。插件中的个别条目呈现为影子条目。插件可以包含JavaScript模块、样式表和模板。插件也可以用于散布普通的文本、图片或其他内容。
  （扩展内容：插件机制）
...
- 样式条目：标题为，\$:/\<name\>/stylesheet/\<name\>。为条目添加样式表系统标签（系统标签：\$:/tags/Stylesheet），在里面写的
  CSS 就会自动应用到全局样式上，并覆盖别的 CSS
  ，详细说明访问太微中文教程：添加css样式。

- 配置条目：标题为，\$:/config/\<name\>

- 调色板条目，标题为\$:/palettes/\<name\>，需要添加\$:/tags/Palette系统标签使其生效。

可选的知识：#link("https://tw-cn.netlify.app/#%E5%AD%97%E6%AE%B5%E5%88%B0%E5%BA%95%E6%98%AF%E4%BB%80%E4%B9%88%E5%91%A2%EF%BC%9F")[字段到底是什么呢？——是什么组成了条目。]
...
点击控制台按钮打开控制台。在控制台 - 插件选项卡 - 点击获取更多插件按钮 -
...
在控制面板的信息选项卡 -
高级组中点击【样式表】，可以看到所有系统预设或者自己自定义的样式，你可以快速在此处找到自己创建的样式并修改。
...
+ TiGi（开箱即用发行版）与ItonNote Wiki模板

+ TiddlyWiki Xp（快速体验TiddlyWiki）

+ TiddlyMemo墨屉（渐进学习与记忆）
...
Tiddlywiki-xp 主页：#link("https://keatonlao.github.io/tiddlywiki-xp/")

#strong[墨屉关于英文原著的知识自建单词库。]
...
- 墨屉主页：#link("https://tiddlymemo.org/manual/zh-Hans")
...
#strong[掌握少量的快捷键有助于提高效率。]

#underline[关于条目的快捷键：alt-N（新建）、ctrl-Enter（保存条目）、Esc（取消保存）]

#underline[保存wiki快捷键：Ctrl +
S、从条目标题跳转到条目内容快捷键：TAB]

#underline[双击编辑条目插件：2click2edit]
...
#align(center)[#table(
  ...
  inset: 6pt,
  [显示效果],
  [#emph[北国]风光，#underline[千里冰封]，万里雪飘。

  望长城#strong[内外]，惟余莽莽；

  大河#super[上标]上下，顿失滔滔。

  山舞银蛇，原驰蜡象#sub[下标]，欲与天公试比高。

  须晴日，看红妆素裹，#strike[分外]妖娆。
...
#align(center)[#table(
  ...
  显示效果：#underline[下划线文本]

  ],
  [#strong[上标]，^^两个英文上角符号^^用于 #strong[#super[上标]] 文本 (
  #strong[X#super[2 ]])
...
需要更加丰富的功能？没问题，下面让我们为你介绍段落格式，如何使用项目列表、引述块、水平分割线破折号以及最为有用和重要的双链功能——#strong[嵌入和链接]（显示入链和出链功能需要安装插件，我们将会在下一节为您介绍）。
...
===== 1）项目列表与引述块的简单介绍
<项目列表与引述块的简单介绍>
为了文档的简易性，我们只列举项目列表与引述块比较常见的使用方式。详细的用法可以访问链接查看：#link("https://keatonlao.github.io/tiddlywiki-xp/#WikiText")
...
#strong[数学公式：]使用一对 #strong[\$\$]
包裹起来的katex公式。需要tiddlywiki/katex插件，太记默认预装。

$ mat(delim: "[", a, b; c, d) $

#strong[代码块：]段落使用 #strong[\`\`\`] 开头和 #strong[\`\`\`]
结尾作为代码块，行内代码使用 \`代码内容\`。注：【 #strong[\`]
】输入方法为英文状态下tab上面的按键。
...
在太微中，你可以插入更加丰富的功能，比如音频、视频、地图、HTML
代码、样式和类、宏、小工具、变量等等。这些都可与WikiText混合使用。它们可以动态的为你组合并显示更加丰富的内容。但这并不是本文的重点，因此只是在此列举一二。

===== 3）自定义样式段落块、类型段落块
<自定义样式段落块类型段落块>
#strong[自定义样式段落块：]使用\@\@以定义的CSS名称或具体CSS样式属性，例如
color，每个后跟 ; 分号，可以在开头的 \@\@
之后立即引入。这样做可以让被包裹起来的元素应用定义的CSS样式表，使段落表现起来更好看或具有特定显示风格。

附加：硬换行CSS，相当于硬换行内容块一样的效果： white-space: pre-wrap;

#strong[类型区块（类型文本块）：]可以用明确的内容类型呈现文本区块，像这样。
...
#align(center)[#table(
  ...
  inset: 6pt,
  [#strong[单元1]], [#strong[单元2]],
  [单元3],
  [单元3],
)
]

更多的功能与需求例如单元合并、单元格垂直对齐、单元合并等内容不在详细介绍，需要的时候可以查阅tiddlywiki
xp文档。（渐进学习）
...
+ 属于区块解析器模式的维基文本符号：标题、段落、表格、引言、硬式换行、水平分隔线、代码区块、定义、清单、样式及
  CSS 类别、类型区块。

+ 维基文本符号例子说明：标题 ，文本符号整行以 ! 开头的

内联解析器模式

+ 这些维基文本类型，可以在没有整行文本的情况下表达。它们不需要全部在一行上，只是它们在一行中表示。因此，单一行中可以出现多个。
  换句话说，当解析器尝试查找特定维基文本的开始和结束位置时，不涉及行尾，当解析器处于内联模式时，它将识别这些维基文本类型的符号：
...
+ 常见的编译指示文本例如：\\define（定义一个宏）、\\whitespace
  trim（裁减文本的起始和结尾的空格）。
...
=== （二）“文件目录”侧边栏
<二文件目录侧边栏>
TiddlyWiki中文件目录的层级结构是使用的Locator插件实现的。在太微中条目的隶属关系是通过【同名条目的标签】实现的。一般来说子条目通常有父条目同名的标签表明隶属关系。比如：子条目名为文件夹，父条目名为目录。只要文件夹打上目录的标签，那么文件夹就隶属于目录，这样层级结构就建立好了。
...
TidGi预安装了bimlas/kin-filterr插件，你可以在
“高级搜索（快捷键：Ctrl+shift+A）-
筛选”选项卡中输入筛选表达式，然后点击旁边的按钮执行批量删除，如下图所示。
...
==== （1）嵌入和链接（双链，重要）
<嵌入和链接双链重要>
#link("https://keatonlao.github.io/tiddlywiki-xp/#WikiText")

#strong[1.1嵌入（Transclusion）：{{两个花括号}}]

您可以使用嵌入符号将一个条目的内容合并到另一个条目的内容中：

{{MyTiddler}} 嵌入一个单独的条目，结果是 MyTiddler 条目的 text
字段（文本字段，即主要内容字段）的内容，被呈现在当前条目内。
...
#strong[1.2 链接（Linking）：\[\[两个方括号\]\]]

#link("https://keatonlao.github.io/tiddlywiki-xp/#WikiText")[WikiText]的关键功能是能够链接到其他条目或外部网站。
...
您还可以从编辑器工具栏创建链接。
点击#strong[链接]，然后搜索并选择一个条目。

#strong[1.2.1驼峰式（CamelCase）链接]
...
==== （3）图像的使用
<图像的使用>
TiddlyWiki的拖动资源功能可以让你将任意的资料拖放进wiki。比如条目、图片、Json格式的TiddlyWiki插件等等。
...
#align(center)[#table(
  ...
  [align],
  [可选],
  [left、、、 和图像的对齐方式，默认值为right center none none],
  [caption],
  [可选],
  [图像标题显示在图像下方],
  [tooltip],
...
使用kookma/commander功能，输入筛选器表达式\[!is\[system\]sort\[title\]\]自动筛选出所有非系统条目，然后点击Export
tiddlers选项卡-导出条目按钮，选择JSON文件格式保存到本地。导出条目文件会保存原有的结构（条目的隶属关系、双向链接以及固定的文本内容图片等不会改变，图片等在条目中使用相对路径引用的文件注意在导入到新的TiddlyWiki中要保持图片的相对路径不要改变。）

拖动导出的JSON文件到新的TiddlyWiki中，单击导入完成条目的转移工作。条目的转移工作不包含之前的用户设置，仅包含普通条目内容，即用户自己编写的全部内容。
...
说明：\[all\[tiddlers\]!is\[system\]\]
通常可以被简写。表示筛选出所有非系统条目。

- 如何筛选出同时拥有某些标签交集的条目？

说明：筛选出同时具有“标签A”和“标签B”的条目。

- 如何筛选出同时拥有标签A或标签B的所有条目？

说明：筛选出只要有“标签A”或“标签B”的条目都会被筛选出来。注意两个表达式之间有空格。

- 如何筛选出标签大于等于两个的条目?

说明：\[all\[tiddlers\]!is\[system\]\]
限定条目为普通条目，compare:number:gteq\[2\]
限定标签条件大于2，可以自行修改。这个表达式可以有变形式，限定标签在一定的范围或小于固定值，请查阅compare筛选器操作符。

- 条目总量如何显示？数字化呈现？

说明：表达式在高级搜索里检索出来的结果为#strong[显示出来的数字]，这就是条目的总数。在控制台
– 信息 – 基础选项卡的最末尾处有显示各类条目的数量。

其它的常用筛选表达式可以在高级搜索（快捷键ctrl-shift-A）筛选选项卡的搜索框旁边的功能按钮找到。
...
<三如何使用tiddlywiki构建笔记系统>
#strong[工具VS内容]

本节主要说明如何运用好【条目】这个信息载体，它可以是卡片，可以是一篇文章，可以是信息节点，可以是一个自动化的信息处理工具等等。如何使用TiddlyWiki因人而异，所以本节内容只提供一个参考和灵感的来源并做一个简单的介绍。相信朋友们的创造力为MAX！
...
#strong[一个有趣的以标签和标题为线索的整理方式：]

使用标签作为线索，线索分为两段，第一段是层级隶属关系，第二段是线索隶属关系。

条目作为文件夹（分类），200-201书籍-20101待读书籍。（这一段是文件夹层级隶属关系），20101中包含书籍列表。书籍列表使用标签标记从属关系，不使用文件夹的从属关系。

=== （二）主题渐构笔记法
<二主题渐构笔记法>
元素渐构笔记法就是关于马的东西放到以马为名称的小卡片中、归类整理即聚类与分类。这个过程就是聚沙成山的过程，滴水石串的累积。这也是运用自己学到的零散或系统知识打造成一柄柄趁手的武器的过程。也是不断扩展自己认知边界发现以前从未发现的探索之旅，对于我们来说，在认知之外事物，由于超出感知与想象的范围，所以基本等价于不存在。元素渐构笔记法，无非就是在不断的累积之中扩展自己的认知圆圈的半径。就像哲学家芝诺说，他曾画了一个圆圈，圆圈内是已掌握的知识，圆圈外是浩瀚无边的未知世界。知识越多，圆圈越大，圆周自然也越长，这样它的边沿与外界空白的接触面也越大，因此未知部分当然显得就更多了。#strong[#underline[元素渐构笔记法就是描述这种从点到圆的渐进过程。]]
...
+ 在父条目内容中链接子类，通过链接创建的子类，手动赋予父类标签。
...
一开始整理笔记，或许不知道如何整理了。这些是因为你缺少了相应的内容补充，也就是少了程序或者插件。举个例子，比如涉及到近现代的西方美学转向问题，那么就需要一定基础对西方绘画艺术有所了解。具体例子比如绘画和摄影。照相机刚发明出来的时候没有人去思考两者内部的关系，只是觉得照相机可以替代绘画了，以后的人不用学画画了。但画画还在发展，20世纪典型代表人物就是毕加索。颠覆了很多艺术之前的准则。那现在就出现了问题，为什么呢？画画和摄影之间的关系是什么呢？摹仿论最早可以追溯到亚里士多德，在绘画领域长时间占据着重要影响，正因为摹仿论，当时才会认为绘画已经被照相机给终结了。但现代主义的画家们纷纷要求改变这一思想，表现更为复杂的现实和人的心理活动。

主题就是一个以你自己主观想要研究的比较感兴趣的关键词，比如研究生命科学的，可以是很大的，人体啊。也可以是研究某一类型的昆虫的生物价值。

想法的产生的大概原因是来源于输入 + 问题 +
反思（思考）。输入就是你读过的、见过的、感受过的等等的任意的事物或信息。这些信息会在我们潜意识进行处理，与已有的知识和经验进行关联，当一个问题暂时无法处理时，一直没有头绪，不妨放一放，等一等。可以画画思维导图，或者在笔记上写出关键词，然后看看关键词之间的联系。等差不多的时候，就去睡觉，不用管了。等到第二天或者第三天。或许你就突然的明白是怎么回事了。
...
太微中，实现可视化的插件有两种可使用，第一个是Echart，第二种是TiddlyMap。其中Echart可以不仅实现条目关系的可视化，还可以实现各种excel可以实现的图表。TiddlyMap可以实现思维导图或关系图谱的信息组织形式。
...
创建新画布，单击打开菜单按钮 -
创建一个新画布，输入新视图名称，点击完成创建画布

在画布中【双击】创建新的节点。创建节点间关联，点击Add
Edge按钮，选中开始始节点并拖放到目标节点完成关联（其中会提示填写边类型，你可以自定义）。

创建的节点会在太微中以条目的形式存储，同样新建条目也是在Map中新建节点，节点之间的联系同样可以使用条目顶部的按钮实现关联，所以TiddlyMap即是关系图谱又是思维导图。它的边和节点的样式都是可以修改的。

控制按钮区域的打开菜单按钮 - 全局配置 或者 Map中 关于边和节点的解释。

Edge-Type：边的类型（边的样式），就是决定边上显示的文字，完全自定义，比如：被引用或者引用。

Node-Type：节点的类型（节点样式），规定节点显示的样式的。就是节点的样式表。

View configuration视图配置，可以配置节点过滤器，用于过滤显示节点。

#underline[更改显示层级按钮的的解释]
...
一个条目可以当做一个卡片。每个卡片之间可以相互嵌入对方的内容，也可以使用链接引用。形成相互关联的卡片关系网，你可以使用Echart的TheBrian视图观察条目节点之间的联系（全局视图与局部视图），也可以通过目录宏生成像目录一样的关系图。也可以对一个条目中的全部标题生成目录，你可以使用标题插件Gk0Wk/page-toc对一个条目卡片生成标题或者使用Section
Editor（部分编辑）根据标题编辑内容。这两个插件在TOC自动化插件里已经介绍过了。
...
输入的时候可以彻底的不整理，就完全堆在日志条目里 然后用 \`\[\[\]\]\`
打个标记，然后用双链的一些特性，试图让后续需要整理的时候压力也不至于很大。

有些东西可能很长一段时间都用不到，那你不去整理它，后续整理的成本也就不用支付了。开始就是随意堆放的，输入成本也很低，就很赚。

#strong[02.太微的自动化无压记录]
...
+ 输入标题「变量：显示或隐藏空节点」，加上标签「logseq/配置」，输入内容
...
但我的判断是，这种情况下一两下点击的差别其实不大（一下点击打开日志，或者两下点击创建新想法及其标题），重点是心理上放心这个内容不会丢。这种放心感才是
rr 双链系统的核心，只要有这种放心感，交互上区别其实可以忽略。

而之所以有信心就算随便堆放的信息，自己也能再找到，是因为【钓鱼插件】，能够定期让我们回顾收件箱中的内容。（详见
#link("https://zhuanlan.zhihu.com/p/442485060")[为临时性的和不完整的笔记设置写作收件箱 - Thoughts Memo 汉化组译制]），这是太微远超
roam research 和 Obsidian 的地方：
...
=== （七）随手记录，动态生成
<七随手记录动态生成>
这个是太微独有的。因为有很多小部件和宏啊。都可以直接在WikiText中的书写文本使用。（例如：教程主页就是动态生成的，你可以随手打开条目编辑看看。都是现成的案例可以学习。）

=== （八）卡片笔记写作法
<八卡片笔记写作法>
知乎作者：亦明Reading

#strong[卡片笔记写作法，实际上就是碎片系统化。]一个卡片就相当于一个条目。条目卡片小鱼。小鱼之间的相互引用。

它能起作用，靠的是围绕主题进行持续性自然积累。
所谓自然积累，即是想到了遇到了就随手记录一下，不需要专门花大块时间。这样也就最大程度的降低了参与门槛。
持续这个动作必然会带来数量上的累积与查找上的麻烦。前者是优势，后者是问题。解决这个问题，靠的是“检索”。
...
P.A.R.A.
代表#strong[项目（Projects）— 领域(Areas) — 资源(Resources) — 档案(Archives)，]四个顶级类别，
包括您在工作和生活中可能遇到的每种类型的信息。

=== （十）书籍与书籍的分卷
<十书籍与书籍的分卷>
关于书籍的分卷的两个插件相关插件。

+ Searchwikis Plugin（搜索维基插件）：通过索引搜索多个维基

主页：#link("https://kookma.github.io/TW-Searchwikis/")

安装Searchwikis
插件的TiddlyWiki可以作为一个检索工具，只有在需要检索的时候才使用它。

#block[
#set enum(numbering: "1.", start: 2)
+ 在Nodejs中可以使用TiddlyServer，点击链接查看#link("https://bramchen.github.io/tw5-docs/zh-Hans/#TiddlyServer%20by%20Arlen%20Beiler")[TiddlyServer]的具体介绍。
]

主页：#link("https://arlen22.github.io/tiddlyserver/")

=== （十一）分享和发布功能
<十一分享和发布功能>
TiddlyWiki最大的一个特性是一个HTML文件，笔记数据会首先保存到HTML文件中。这意味着发布一个静态的网站就像打印一篇公告一样简单。只需要把它放到公告栏的位置。比如GitHub的gh-pages（GitHub
页面）。然而你也可以把它发给别人直接打开。

TiddlyHost，一个在线的站点可以允许用户编辑保存，仅需要注册账号即可使用，你可以把它当做发布页。

分享wiki的一条笔记可以选择导出为HTML文件，如果对方有太微，可以选择JSON格式文件，方便对方导入。如果是多条笔记，可以使用筛选器，筛选出符号条件的条目导出。

=== （十二）数据安全性与备份
<十二数据安全性与备份>
TiddlyWiki的数据安全性措施有草稿功能自动保存功能，简单的说，只要你确认保存了就不用担心数据会突然丢失，改动会写入文件内，就算是插件导致的功能性损坏不可用，也可以找回来，除非这个文件被删除或物理损坏。要保证的是，改动一定存到文件系统上了才行。

不过也有备份的措施比如：TidGi（太记）的历史版本工具、TiddlyDesktop的备份管理，然后是桌面端的KopiaUI自动定时备份。
...
<git-提交更改commit-changes>
当文件被更改后，一般会显示在这里。然后你可以使用Add（添加）将变更的文件添加到暂存区，在使用Commit（提交）将更改（Changes）放入版本库(Repository)的当前分支上。在GithubDesktop中只需要在summary中输入总结并点击
Commit to
master按钮即可完成这两步——将更改的文件放入版本库的当前分支上。之后就可以在History（历史记录）中查看刚刚提交的修改。
...
== 四、为知识库拓展新的功能与特性
<四为知识库拓展新的功能与特性>
太微独一无二的优势就是生来即是网页，可以无需额外软件发布给任何人，寿命与前端三件套齐长。自由度非常高，只要你想，连标记文本都是可以由用户定义的。因此也可以把TiddlyWiki看做对知识进行管理的操作系统，TiddlyWiki插件作为系统的应用，而底层数据就是条目。
...
#strong[如无必要，勿增实体。功能越多，问题越多。简单实用稳定就是最好的。]
...
=== （一）Markdown插件
<一markdown插件>
在官方库搜索Markdown插件，点击安装。然后再CPL搜索Markdown-Export，点击安装。注意
，这两个插件是都要装的，不然导入的markdown文件无法正常显示。

可以把新建Markdown的按钮放在侧边区域的页面控制栏，设置可以在控制台 -
视觉外观 - 工具栏 - 页面工具栏启用【添加Markdown条目】
...
说明：kiasu/RandomTiddlerButton、tobibeer/random两个插件都需要安装，安装后请刷新wiki。
...
=== （六）与SuperMemo互动
<六与supermemo互动>
你可以在SuperMemo中导出HTML文件，在浏览器中经过简单的处理，就可以导入太微中，构建自己的小鱼。
...
== 五、帮助与更多
<五帮助与更多>
你可以在以下三个站点找到丰富的资源教程，或者在知乎或B站找到实用的使用方式。我们对新人的建议是，学无止境，够用就好。太微的上限很高，达到一个比较好的效果就可以暂时性的不用学习新的知识。等到有新的功能需求，自己再去探索未知领域。
...
#blockquote[
感谢CREATOR（keatonlao）的wikitext教程、suan、FSpark、TomZheng、凹夫几、林一二、葫芦12、sugarhope、少年游、机杼提供的意见和想法以及每一位太微爱好者的支持，非常感谢！
]
...
关于需要了解的HTML知识（可选）：HTML标签、盒子、选择器、属性与内容、HTML结构、应用CSS到元素的概念以及常用的功能。

=== （一）桌面端Timimi的安装使用
<一桌面端timimi的安装使用>
太微有非常多的保存方式，有一种保存方式Timimi，你可以把它们当做文档与保存文档软件的关系。Timimi仅需配置一次且使用起来几乎感受不到存在。有一个好主意是，你可以使用TiddlyStow搭配Timimi并把它们作为一种日常使用太微单文件版的方式。其中TiddlyStow作为创建太微的角色，Timimi作为保存太微文档的工具。如果担心会丢失数据可以使用Kopia定时备份。

安装流程概述：1.安装浏览器插件（注意，启用选项，允许来自其它应用商店的扩展）、2.打开允许访问文件URL、3.安装主机伺服软件。
...
在太微控制面板 – Info选项卡 – 基本组 ，往下滑找到 “Hello! Current
language” 设置，下拉列表选择
